...
function drawHUD()
	...
	lg.setColor(16,12,9)
	lg.print("PLACAR: "..score,150,187)
	lg.setColor(246,247,221)
	lg.print("PLACAR: "..score,150,186)
	lg.setColor(255,255,255)
...
function drawPrescreen()
	...
	lg.setFont(font.bold)
	if map.type == MT_NORMAL then
		lg.printf("ANDAR ".. floor .. "-" .. floor+2, 0, 40, WIDTH, "center")
		lg.draw(img.captain_dialog, quad.prescreen_music, 7, 183)
	else
		lg.printf("TETO", 0, 40, WIDTH, "center")
	end
	...
	lg.printf("APERTE ENTER PARA CONTINUAR", 0, 150, WIDTH, "center")
end
...
function setPrescreenMessage()
	if casualties >= max_casualties then
		prescreen_message = "MUITOS CIVIS MORRERAM!\n\nESTA DEMITIDO!"
	elseif player.state == PS_DEAD then
		prescreen_message = "SEU TRAJE SUPER-AQUECEU!\n\nFIM DE JOGO"
	else
		if section == 1 then
		...
		elseif last_missed > 0 then
			if last_missed == 1 then
				prescreen_message = "EI, CARA!\nSE ESQUECEU DE UMA PESSOA.\nTENHA CUIDADO."
			else
				prescreen_message = "EI, CARA!\nVOCE DEIXOU "..last_missed.." PESSOAS PARA TRAS.\nTENHA CUIDADO."
			end
		else
